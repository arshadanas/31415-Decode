 ...
 */
public final class LaserRangefinder {
    ...
    public void setDistanceMode(LaserRangefinder.DistanceMode mode) {
        write(R_DISTMODE, new byte[]{(byte) (mode.ordinal() + 1)});
    }

    public LaserRangefinder.DistanceMode getDistanceMode() {
        byte v = i2c.read8(R_DISTMODE);
        switch (v) {
            case 1:
                return LaserRangefinder.DistanceMode.SHORT;
            case 2:
                return LaserRangefinder.DistanceMode.MEDIUM;
            case 3:
                return LaserRangefinder.DistanceMode.LONG;
            default:
                throw new RuntimeException("Could not get distance mode, " + v);
        }
    }
    ...
    public void setTiming(int budget, int period) {
        if (budget < 5 || budget > 1000)
            throw new RuntimeException("Invalid timing budget: " + budget);
        if (period != 0 && (period < budget + 3))
            throw new RuntimeException("Measurement period must be at least 4ms more than timing budget if set. For fast ranging, use a period of 0.");
    ...
    public void setROI(int topLeftX, int topLeftY, int botRightX, int botRightY) {
        if (botRightX - topLeftX < 3 || topLeftY - botRightY < 3)
            throw new RuntimeException("ROI too small, must be at least 4x4.");
        for (int v : new int[]{topLeftX, topLeftY, botRightX, botRightY})
            if (v < 0 || v > 15) throw new RuntimeException("Invalid ROI point value: " + v);
    ...
    public void setI2CAddress(int newAddress) {
        if (newAddress < 1 || newAddress > 127)
            throw new RuntimeException("Invalid I2C address: " + newAddress);
    ...
    public LaserRangefinder.ScanSequenceBuilder setAnalogScanMode() {
        return new LaserRangefinder.ScanSequenceBuilder(M_AN2);
    }

    public LaserRangefinder.ScanSequenceBuilder setI2CScanMode() {
        return new LaserRangefinder.ScanSequenceBuilder(M_II2);
    }
    ...
    public class ScanSequenceBuilder {
        ...
        public LaserRangefinder.ScanSequenceBuilder addScanROI(
                int topLeftX,
                int topLeftY,
                int botRightX,
                int botRightY
        ...
        public void setScanROIs() {
            for (byte[] roi : rois)
                i2c.write(R_PIN0_MODE, new byte[]{mode, roi[0], roi[1], roi[2], roi[3]});
